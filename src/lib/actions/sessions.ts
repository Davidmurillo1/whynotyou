...
export type SessionResult =
  | { error: string }
  | { ok: true; itemCompleted: boolean; sessionId: string }

export type SessionHighlightResult =
  | { error: string }
...
export async function createSessionAction(input: {
  ...
  try {
    ...
    return { ok: true, itemCompleted, sessionId }
  } catch {
    return { error: 'No pudimos guardar la sesión. Tu tiempo está guardado, podés reintentar.' }
  }
}

export async function getSessionHighlightAction(
  sessionId: string,
): Promise<SessionHighlightResult> {
  try {
    const supabase = await createSupabaseServerClient()
    const {
      data: { user },
    ...
    const { data: session } = await supabase
      .from('sessions')
      .select('id, item_id, duration_seconds, units_progressed, user_id')
      .eq('id', sessionId)
      .single()

    if (!session || session.user_id !== user.id) {
      return { error: 'No encontramos esa sesión.' }
    }

    const { data: item } = await supabase
      .from('items')
      .select('id, total_units, current_units')
    ...
    const currentUnits = Number(item.current_units)
    let itemCompleted = currentUnits >= totalUnits
    ...
    return { error: 'No pudimos calcular el highlight.' }
  }
}
