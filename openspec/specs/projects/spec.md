...
Permite al usuario agrupar ítems de aprendizaje bajo proyectos transversales. Un proyecto es un contenedor N:M sobre ítems, con su propio nombre, color, emoji y estado (`active` | `archived`). La feature expone páginas dedicadas (`/proyectos`, `/proyectos/[id]`), integra visibilidad en la biblioteca y el detalle del ítem, y agrega una entrada en la navegación principal.
